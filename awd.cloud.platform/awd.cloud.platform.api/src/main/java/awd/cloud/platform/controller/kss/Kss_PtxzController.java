...
@RestController
@RefreshScope
@RequestMapping("/v4/kss/ptxz")
@Api(tags = "kss-ptxz",description = "ptxz")
public class Kss_PtxzController extends PublicService {

    @Autowired
    private KssServerApis kssServerApis;

    /**
     * @api {get} /v4/kss/ptxz/ptxzList 陪同巡诊查询
     * @apiVersion 0.4.0
     * @apiName ptxzList
     * @apiGroup g_kss
     * @apiPermission any
     * @apiDescription 陪同巡诊查询
     *
     ...
     * @apiSuccess {String}dcmdaij          				                    带出民警
     ...
     *          "bzlx": "陪同巡诊",
     *          "bzlxString": null,
     *          "kssj": "2019-12-30 11:56:36",
     *          "jssj": "2019-12-30 11:56:37",
     *          "dcsj": null,
     *          "dcmj": "",
     *          "dcrs": "0",
     *          "qy:" "ccc",
     ...
     *          "bz": "",
     *          "creator": "管理员",
     *          "createtime": "2019-12-30 11:56:46",
     *          "updator": "",
     *          "updatetime": null
     *       }
     *     ],
    ...
    public ResponseMessage<Map<String,Object>> ptxzList(HttpServletRequest request, @RequestParam(name = "appcode")String appcode, @RequestParam(name = "jsbh")String jsbh, String json){

        String interfaceId = "/v4/kss/ptxz/ptxzList";
        ...
        try {
            ...
            ResponseMessage<PagerResult<QybzModel>> result = kssServerApis.ptxzList(param);
    ...
    /**
     * @api {post} /v4/kss/ptxz/ptxzAdd 陪同巡诊保存
     * @apiVersion 0.4.0
     * @apiName ptxzAdd
     * @apiGroup g_kss
     * @apiPermission user
     *
     * @apiDescription 陪同巡诊保存
     *
     * @apiParam {String} appcode 						    应用代码（必填）
     ...
     *
     */
    //{"mjxm":".{1,60}","kssj":"^(\\d{4})-([0-1]\\d)-([0-3]\\d)\\s([0-5]\\d):([0-5]\\d):([0-5]\\d)$","jssj":"^(\\d{4})-([0-1]\\d)-([0-3]\\d)\\s([0-5]\\d):([0-5]\\d):([0-5]\\d)$","bzlx":".{1,20}","creator":".{1,30}"}
    @ApiOperation("陪同巡诊保存")
    @PostMapping("/ptxzAdd")
    @HystrixCommand
    @ApiResponses({@ApiResponse(code = 200, message = "查询成功"), @ApiResponse(code = 401, message = "未授权"),
            @ApiResponse(code = 403, message = "无权限"), @ApiResponse(code = 404, message = "数据不存在")})
    @OpenAPI
    public ResponseMessage<String> ptxzAdd(HttpServletRequest request, @RequestParam(name = "appcode") String appcode, @RequestParam(name = "jsbh") String jsbh, String json) {
        //接口id
        String interfaceId = "/v4/kss/ptxz/ptxzAdd";
        try {
            //校验权限
            ResponseMessage<Map<String, Object>> maps = this.userAuthorizatio(request, json, interfaceId);
            if (maps.getStatus() != 200) {
            ...
            ResponseMessage<String> result =   kssServerApis.ptxzAdd(qybzModel);
