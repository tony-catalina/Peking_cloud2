define(function (require) {
  ...
  var config =
    ...
    ChartSankey = require("./modules/charts/appManage/ChartSankey"),
  ...
  var gainLTFn = function (el, cb) {
    basefn(el, {
      App: App,
      desc: "left top",
      url: config.interface.jls_zyjbxx + "?rybh=" + ryxm,
  ...
  var gainLCFn = function (el, cb) {
    basefn(el, {
      App: App,
      desc: "left center",
      url: config.interface.jls_ryqk + "?jsbh=" + idjsbhxf + "&rybh=" + ryxm,
      useLoading: basedata.mark_.loadFirst,
      cb: function (resp) { 
        // console.log(resp,147258)
        resp = resp.manage.lc;
  ...
  var gainLBFn = function (el, series, cb) {
    var url="";
    if(series == "0"){
		  url = config.interface.jls_zyrjjkqk;
	  }else if(series == "1"){
		  url = config.interface.jls_zyrwgsjcl;
	  }
    basefn(el, {
      App: App,
      desc: "left bottom",
      url: url + "?jsbh=" + idjsbhxf + "&jsh=" + ryjsh + "&rybh=" + ryxm,
      useLoading: basedata.mark_.loadFirst,
      cb: function (resp) {
        // console.log(resp)
        resp1 = resp;
        ...
        $ft.text('');
        $ft.text('');
        // var $resp = resp1.filter(item => item.name == series)[0].list
        cb && cb(el, resp1)
      }
    })
  };
  ...
  var gainRTFn = function (el, cb) {
    basefn(el, {
      App: App,
      desc: "right top",
      url: config.interface.jls_rygx + "?jsbh=" + idjsbhxf + "&jsh=" + ryjsh + "&rybh=" + ryxm,
      useLoading: basedata.mark_.loadFirst,
      cb: function (resp) {
        // console.log(resp,111222333)
        console.log(resp.data.manage.rt,456789)
  ...
  var gainRBFn = function (el, cb) {
    basefn(el, {
      App: App,
      desc: "right bottom",
      url: config.interface.jls_flsxbg+"?jsbh=" + idjsbhxf + "&jsh=" + ryjsh + "&rybh=" + ryxm,
      useLoading: basedata.mark_.loadFirst,
      cb: function (resp) {
        resp = resp;
        cb && cb(el, resp)
  ...
  var gainCTRFn = function (el, cb) {
    basefn(el, {
      ...
      url: config.interface.jls_fxdj + "?rybh=" + ryxm,
      useLoading: basedata.mark_.loadFirst,
      cb: function (resp) {
  ...
  var renderCBFn = function (el, resp) {
    ...
    $(".sex-left").on("click", function () {
      ...
      // if (resp.find(item => item.name == series).list) {
      //   var $resp = resp.find(item => item.name == series).list
      //   renderLBFn("box2", $resp)
      // }
      if(series == "健康情况"){
  ...
  var gainCBFn = function (el, cb) {
    basefn(el, {
      App: App,
      desc: "center bottom",
      url: config.interface.jls_fxys + "?jsbh=" + idjsbhxf + "&jsh=" + ryjsh + "&rybh=" + ryxm,
      useLoading: basedata.mark_.loadFirst,
      cb: function (resp) {
        console.log(resp,456456)
        resp = resp.data;
        var $resp = [];
        _.each(resp, function (itm, idx) {
          $resp.push({
