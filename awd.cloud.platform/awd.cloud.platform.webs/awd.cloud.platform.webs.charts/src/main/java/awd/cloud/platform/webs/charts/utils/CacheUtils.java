...
public class CacheUtils extends ConstantUtils {
    ...
    public static final String AWD_CHARTS ="AWDCHARTS_";
    ...
    public void setTempMessage(String msgID, String content) {
        redisUtils.set(AWD_CHARTS + msgID, content,1800L);
    }

    public List<String> getTempMessage(User user) {
        Set<String> set = redisUtils.getkeys("AWDCHARTS_"+user.getJsbh()+user.getUsername()+"_*");
        List<String> result = new ArrayList<>();
        for (String key : set
        ) {
            result.add(redisUtils.get(key));
            redisUtils.remove(key);
        }
        return result;
    }

    public static List<String> getInfoListBykey(String keys) {

        //String keys = CacheUtils.CACHE_KSS_JQ + jsbh;
        Set<String> stringSet = getKeys("AWDCHARTS_"+keys+"_*");
        System.out.println("=================000:"+"AWDCHARTS_"+keys+"_*");
        System.out.println("===========00"+stringSet.toString());
        List<String> jsonObjectList = new ArrayList<>();
        //stringSet.forEach(p -> jsonObjectList.add(JSONObject.parseObject(redisUtils.get(p))));
        for (String key : stringSet
                ) {
            jsonObjectList.add(redisUtils.get(key));
            redisUtils.remove(key);
        }

        System.out.println("=======1"+jsonObjectList);
        return jsonObjectList;
