define("app/src/message/fileContent", [
...
//		if(G.user){
//			ajaxurl = "share/pathList?user=" + G.user + "&sid=" + G.sid + "&path=" + urlEncode(url);
//		}
		$.ajax({
			url: ajaxurl,
			dataType: "json",
	...
	pathTypeChange = function(e) {
		if (e.info) {
			...
			t == G.AWD_USER_RECYCLE ? (o.removeClass(n).addClass("menuRecycleBody"), $(".tools-left>.btn-group").addClass("hidden").parent().find(".awd_recycle_tool").removeClass("hidden")) : -1 !== s.indexOf(t) ? -1 === core.pathClear(rtrim(G.this_path, "/")).indexOf("/") ? (o.removeClass(n).addClass("menuShareBody"), $(".tools-left>.btn-group").addClass("hidden").parent().find(".awd_share_tool").removeClass("hidden"), i.id == G.user_id ? ($(".menuSharePathMenu").find(".open_the_path,.share_edit,.remove").removeClass("hidden"), $(".menuSharePathMore").find(".remove").removeClass("hidden")) : ($(".menuSharePathMenu").find(".open_the_path,.share_edit,.remove").addClass("hidden"), $(".menuSharePathMore").find(".remove").addClass("hidden"))) : (o.removeClass(n).addClass("menuBodyMain"), $(".tools-left>.btn-group").addClass("hidden").parent().find(".awd_path_tool").removeClass("hidden")) : (o.removeClass(n).addClass("menuBodyMain"), $(".tools-left>.btn-group").addClass("hidden").parent().find(".awd_path_tool").removeClass("hidden")),
			currentPathMenu(e)
		}
	},
	currentPathMenu = function(e) {
		...
		o = "disable";
		if (i.can_upload ? ($("ul.menufolder,ul.menuMore,ul.menufile,ul.fileContiner_menu").find(n).removeClass(o), $(".path_tips").hide(), $(".awd_path_tool>button").removeClass("disabled")) : ($(".awd_path_tool>button").addClass("disabled"), $("ul.menufolder,ul.menuMore,ul.menufile,ul.fileContiner_menu").find(n).addClass(o), $(".path_tips span").html("只读"), a == G.AWD_USER_RECYCLE || a == G.AWD_USER_SHARE ? ($(".path_tips").hide(), $(".awd_path_tool>button").removeClass("disabled"), a == G.AWD_USER_SHARE && G.user_id != i.id && $(".awd_path_tool>button").addClass("disabled")) : $(".path_tips").show()), (a == G.AWD_GROUP_PATH || a == G.AWD_GROUP_SHARE) && G.is_root || a == G.AWD_GROUP_PATH && "owner" == i.role) {
