...
var aesDecode = function(str,key){
	console.log("aesDecode")
	console.log(CryptoJS.AES.decrypt(str,key).toString(CryptoJS.enc.Utf8))
...
var ShareData = {
	...
	frameTop:function(frame,action){//frame=='' 则parent;为空则获取；指定则从top找child
		...
		var testParent = function (page) {
			try {
				if(page.parent && page.parent.AWD_NAMESPACE){
					return page.parent;
				}else{
					return false;
				}
...
var MaskView =  (function(){
	...
	var image = function(url){
		add("<img class='awd_image_view_loading' src='"+staticPath+"js/lib/picasa/style/loading.gif' style='position:fixed;top:50%;left:50%;opacity:0.5;z-index:99'/>"+
			"<img src='"+htmlEncode(url)+"' class='image awd_image_view' "+
		...
		$('#maskViewContent .awd_image_view_loading').fadeIn(300);
		$('#maskViewContent .awd_image_view').load(function(){
			$('#maskViewContent .awd_image_view_loading').stop(true).fadeOut(500, function() {
				$(this).remove();
			});
			_resize();
