...
function seajsConcat() {
  ...
  let buildFilter = filter(['**/controller/*.js'].concat(cfg.filter.require), { restore: true });
  ...
  if (cfg.options.isProduction) {
    ...
    if(modRequireCfg){
      ...
      return gulp.src(cfg.src.js, { cwd: path.resolve(cfg.path.app) , base: cfg.path.base })
        ...
        // .pipe(babel())
        .pipe(seajsCombo({
          ignore : [ 'jquery','easyui','Swiper','echarts','sJI','CountUp','UnderScore' ],
          map : {
                  jquery: path.resolve(cfg.path.app)+'/apps/kss_syqt/static/lib/jquery/jquery.min',
                  easyui: path.resolve(cfg.path.app)+'/apps/kss_syqt/static/lib/jquery/jquery.easyui.min',
                  arttemplate: path.resolve(cfg.path.app)+'/apps/jds/static/lib/arttemplate/template-web',
                  Swiper: path.resolve(cfg.path.app)+'/apps/kss_syqt/static/lib/swiper/swiper.min',
                  echarts: path.resolve(cfg.path.app)+'/apps/kss_syqt/static/lib/echarts/echarts.min',
                  sJI: path.resolve(cfg.path.app)+'/apps/kss_syqt/static/lib/class/extends',
                  // 数字滚动
                  CountUp: path.resolve(cfg.path.app)+'/apps/kss_syqt/static/lib/countUp/countUp.min',
                  UnderScore: path.resolve(cfg.path.app)+'/apps/kss_syqt/static/lib/underscore/underscore-min'
                }
        }))
        .pipe(uglify())
        .pipe(jsFilter.restore)
