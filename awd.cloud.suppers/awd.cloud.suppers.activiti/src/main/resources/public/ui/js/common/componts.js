define("/ui/js/common/componts", [
        "/ui/js/plugins/easyui/jquery-easyui-1.5.1/jquery.easyui.min",
        "/ui/js/plugins/easyui/jquery-easyui-1.5.1/locale/easyui-lang-zh_CN"
    ],
    function (require) {
        var _initTable = function (id, options) {
            ...
            var _method = options['method'] != undefined ? options['method'] : 'get';
            ...
            var _pageSize = options['pageSize'] != undefined ? options['pageSize'] : '50';
            ...
            var _onExpandRow = options['onExpandRow'] != undefined ? options['onExpandRow'] : function () {
            };
            ...
            var _loader = options['loader'] != undefined ? options['loader'] :
            ...
            var _loadFilter = options['loadFilter'] != undefined ? options['loadFilter'] : function (data) {
                if (data && typeof (data) == "object" && data.result) {

                    return {total: data.totalSize, rows: data.result};
                } else {
                    alert("数据有问题!");
                    return {total: 0, rows: []};
                }
        ...
        var _initSelectPage = function (id, callback) {
            var select;
            if (id != undefined) {
                select = "#" + id;
            } else {
            ...
            $(select).each(function () {
                ...
                $(this).combogrid({
                    ...
                    columns: [[{field: 'ck', checkbox: $(this).attr("checkBox") == "true"},
                        ...
                        {field: 'key', title: '编号', width: 80, hidden: true},
                        {field: 'name', title: biaoti, width: 150},
                    ]],
                    onShowPanel: function () {
            ...
            var getData = function (page, rows, fieldName, queryParams, location, alike) {
                var location = location == "" ? "/deployment/process-list" : location;
                ...
                $.ajax({
                    type: "POST",
                    url: location,
                    data: "pageIndex=" + page + "&pageSize=" + rows + "&fieldname$eq=" + fieldName + "&name=" + queryParams,
                    ...
                    success: function (data) {
                        if (alike != "" && alike != undefined && alike != null) {
                            $('input[alike="' + alike + '"]').combogrid('grid').datagrid('loadData', data.result);
                        } else {
                            $('input[code="' + fieldName + '"]').combogrid('grid').datagrid('loadData', data.result);
                        }
        ...
        var _initDropDownTree = function (id) {
            var select;
            if (id != undefined) {
                select = "#" + id;
            } else {
        ...
        var _initTree = function (id, tableid, options) {
            var _url = options['url'] != undefined ? options['url'] : "/models/list/page";
            console.log(_url);
            var _method = options['method'] != undefined ? options['method'] : "get";
            var _loadFilter = options['loadFilter'] != undefined ? options['loadFilter'] : function (data) {
                return data;
            };
            var _onClick = options['onClick'] != undefined ? options['onClick'] : function (node) {
                var data = {};
                /*var flag = $("#s_param")[0];
                if (flag == null || flag == undefined) {
                    ...
                    var class_param = $('#'+tableid).parent().parent().find('.s_param');
                    if (class_param.length > 0) {
                        var s_param = class_param.textbox('getValue');
                    }else {
                        var s_param = $("#s_param").textbox('getValue');
                    }
                    data = {
                            'param': s_param,
                    }
            ...
            $('#' + id).tree({
                ...
                onLoadSuccess: function (node, data) {
                    ...
                    $('#' + id + " .tree-node.treet1").bind("mouseover", function () {
                        trrId = $(this).attr("id");

                        var node = document.getElementById(trrId);
                        var childrenNode = $('#' + id).tree('getNode', node);
                        var qtip = childrenNode.qtip;
                        $('#' + id + " .tree-node.treet1").tooltip({
                            trackMouse: 'true',
        ...
        var _formatter = function (row, index, column, content) {
            return '<span title=' + (row[column] == null ? content : row[column]) + '>'
                + (row[column] == null ? "" : row[column]) + '</span>'
        };
        ...
        var maskInit = function () {
            ...
            if (!$maskMsg) {
                $maskMsg = $("<div class=\"datagrid-mask-msg mymask\" style='z-index: 9999999;'>" + defMsg + "</div>")
        ...
        return {
            ...
            initDropDownTree: _initDropDownTree,
            initTree: _initTree,
