...
public class FileUtil {
	...
	public String makeOracleDir() {
		...
		if (oracle.exists()) {
			...
			if (oracleFlag) {
		    	...
		    	msg = "create root dir : "+ mysqlHomeFolder +" folder is fil failed !";
		    }
		}
		System.err.println(msg);
		return msg;
	...
	/**
	 * 创建从oracle数据库 获取的数据 转成文件 
     * @param filePathAndName 文本文件完整绝对路径及文件名
     * @param fileContent     文本文件内容
     * @return
     */
    public Map<String, Object> createOracleFile(String fileName, String fileContent) {
        ...
        try {
            ...
            System.err.println("createOracleFile...filePath is : "+filePath);
	...
	/**
	 * 获取存在ftp中的 mysql 数据库的 json 文件
     * @param filePathAndName 文本文件完整绝对路径及文件名
     * @param fileContent     文本文件内容
     * @return
     */
	public String downLoadFileByFtp() {
		String sourcePath = mysqlHomeFolder;	//这个是ftp服务器的 源文件目录
		String targetPath = mysqlHomeFolder;	//这个时候本地下载的 目标目录。这里我应该是一样的
	...
	public List<String> getJsonObjList() {
		String sourcePath = mysqlHomeFolder;	//这个是ftp服务器的 源文件目录，准备从这里读取文件
	 ...
	 * @param <T>
	 */
	public <T> List<T> getJsonObjectList(String localFilePath, Class<T> clazz) {
		//把文件读取成string数据,是list<object> 类型的
		String jsonStrList = getJsonStrByFile(localFilePath);
		System.err.println(jsonStrList);
		List<T> jsonList = JSONArray.parseArray(jsonStrList, clazz);
		return jsonList;
	}
	
	/**
	 * 把从ftp服务器读取的string文件换换成java对象
	 * @param <T>
	 */
	/*public <T> T getJsonObjectToBean(String remoteFilePath, Class<T> clazz) {
		String jsonStr = fTPUtils.readFtpFile(remoteFilePath);
